...
class SprintLocoPolicy(Policy):
    """Sprint locomotion policy - retrained with sim-to-real improvements"""
    ...
    def __init__(self, cfg_policy: MjlabLocoPolicyCfg, device):
        ...
        logger.info(f"Loading sprint policy from {cfg_policy.policy_file}")
    ...
    def get_observation(self, env_data, ctrl_data):
        ...
        # Get base_lin_vel from environment
        ...
        obs = np.concatenate([
            ...
            # 7. command (3)
            np.array([
                self.lin_vel_command[0],  # lin_vel_x
                self.lin_vel_command[1],  # lin_vel_y
                self.ang_vel_command[0],  # ang_vel_z
            ], dtype=np.float32),
    ...
    def get_action(self, obs: np.ndarray) -> np.ndarray:
        ...
        # NO per-joint scaling - model was trained to handle waist properly
